...
// john
std::atomic_bool fAddressIndex(false);
std::atomic_bool fTimestampIndex(false);
std::atomic_bool fSpentIndex(false);
...
CAmount GetBlockSubsidy(unsigned int nHeight, const Consensus::Params& consensusParams, CAmount &nTotalRewardWithMasternodes, bool fSuperblockPartOnly, bool fMasternodePartOnly, unsigned int nStartHeight)
{
    ...
    if (!bRegtest && nHeight == 1) {
        // SYSCOIN 4 snapshot
        nTotalRewardWithMasternodes = 3500001 * COIN;
        return nTotalRewardWithMasternodes;
    }

    CAmount nSubsidy = 63.5 * COIN;
    ...
    if(reductions < 3) {
        TwoFundReward = (reductions == 0) ? (1500*10000*COIN)/HalvingInterval : (750*10000*COIN)/HalvingInterval;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, std::unordered_set<std::string> &actorSet, bool fJustCheck)
{
    ...
    if (nBlockHeight > 1) {
        ...
        if (FundEnd < 3) {
            ...
            TwoFundReward = (FundEnd == 0) ? (1500*10000*COIN)/HalvingInterval : (750*10000*COIN)/HalvingInterval;
...
bool static LoadBlockIndexDB(const CChainParams& chainparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    bool fAddressIndexing = false;
    pblocktree->ReadFlag("addressindex", fAddressIndexing);
    if(fAddressIndexing) fAddressIndex = true;

    // Check whether we have a timestamp index
    bool fTimestampIndexing = false;
    pblocktree->ReadFlag("timestampindex", fTimestampIndexing);
    if(fTimestampIndexing) fTimestampIndex = true;

    // Check whether we have a spent index
    bool fSpentIndexing = false;
    pblocktree->ReadFlag("spentindex", fSpentIndexing);
    if(fSpentIndexing) fSpentIndex = true;
    LogPrintf("fSpentIndex: %s\n", fSpentIndex ? "enabled" : "disabled");
