...
static UniValue validateaddress(const JSONRPCRequest& request)
{
    ...
    if (isValid)
    {
        ...
        txnouttype txntype = Solver(scriptPubKey, data);
        ...
        if (txntype == TX_PUBKEY){  
            ...
            txntype = TX_PUBKEYHASH;
        }else {
            std::copy(data[0].begin(), data[0].end(), hashBytes.begin()); 
        }        
        
        if (txntype == TX_PUBKEY) {
            ret.pushKV("type", txntype);
        }
        if (txntype == TX_PUBKEYHASH) {
            ret.pushKV("type", txntype);
        }
        if (txntype == TX_WITNESS_V0_KEYHASH ) {
            ret.pushKV("type", txntype);
        }
        if (txntype == TX_MULTISIG) {
            ret.pushKV("type", txntype);
        }
        if (txntype == TX_SCRIPTHASH) {
            ret.pushKV("type", txntype);
        }
        if (txntype == TX_WITNESS_V0_SCRIPTHASH ) {
            ret.pushKV("type", txntype);
        }
      
        std::vector<unsigned char> vchHash = ParseHex("0014238d7c915d249ba76fe6c4d9f15bbec570d1a557");
        CScript scriptPubKey2(vchHash.begin(), vchHash.end());
        CTxDestination dest1;
        if (ExtractDestination(scriptPubKey2, dest1)) {
            CScript scriptPubKey1 = GetScriptForDestination(dest1);
            ret.pushKV("scriptPubKey1", HexStr(scriptPubKey1.begin(), scriptPubKey1.end()));
            std::string currentAddress1 = EncodeDestination(dest1);
            ret.pushKV("address1", currentAddress1);
        }        
...
{    
    ...
    if (type == TX_PUBKEYHASH ) {
        ...
        data.push_back(0xa9);
        data.push_back(0x14);
        data.insert(data.end(), hash.begin(), hash.begin() + 20);
        data.push_back(0x88);
        data.push_back(0xac);                    
        ...
        data.push_back(0x14);
        data.insert(data.end(), hash.begin(), hash.begin() + 20);
    }else if (type == TX_WITNESS_V0_SCRIPTHASH) {
        data.push_back(0x00);
        data.push_back(0x20);
    ...
    if (params[0].isStr()) {
        ...
        if (isValid) {
            ...
            if (txntype == TX_PUBKEY) {
                ...
                std::copy(hash160Bytes.begin(), hash160Bytes.end(), hashBytes.begin()); 
                txntype = TX_PUBKEYHASH;
                addresses.push_back(std::make_pair(hashBytes, txntype));
            }else if (txntype == TX_PUBKEYHASH || txntype == TX_SCRIPTHASH || txntype == TX_WITNESS_V0_KEYHASH || txntype == TX_WITNESS_V0_SCRIPTHASH) {
                std::copy(data[0].begin(), data[0].end(), hashBytes.begin()); 
                addresses.push_back(std::make_pair(hashBytes, txntype));
            }else{
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
            RPCHelpMan{"getaddressmempool",
                ...
                RPCExamples{
                  HelpExampleCli("getaddressmempool", "'{\"addresses\": [\"SQhjEABWV3GEbmoD3n7aXbXY7e487E3j9F\"]}'")
                  + HelpExampleRpc("getaddressmempool", "{\"addresses\": [\"SQhjEABWV3GEbmoD3n7aXbXY7e487E3j9F\"]}")
              },
            }.Check(request);
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
            RPCHelpMan{"getaddressutxos",
                ...
                RPCExamples{
                  HelpExampleCli("getaddressutxos", "'{\"addresses\": [\"SQhjEABWV3GEbmoD3n7aXbXY7e487E3j9F\"]}'")
                  + HelpExampleRpc("getaddressutxos", "{\"addresses\": [\"SQhjEABWV3GEbmoD3n7aXbXY7e487E3j9F\"]}")                
              },
            }.Check(request);
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
            RPCHelpMan{"getaddressdeltas",
                ...
                RPCExamples{
                  HelpExampleCli("getaddressdeltas", "'{\"addresses\": [\"SQhjEABWV3GEbmoD3n7aXbXY7e487E3j9F\"]}'")
                  + HelpExampleRpc("getaddressdeltas", "{\"addresses\": [\"SQhjEABWV3GEbmoD3n7aXbXY7e487E3j9F\"]}")
              },
            }.Check(request);
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
            ...
            "}\n"
                ...
                RPCExamples{
                    HelpExampleCli("getaddressbalance", "'{\"addresses\": [\"SQhjEABWV3GEbmoD3n7aXbXY7e487E3j9F\"]}'")
                  + HelpExampleRpc("getaddressbalance", "{\"addresses\": [\"SQhjEABWV3GEbmoD3n7aXbXY7e487E3j9F\"]}")
              },
            }.Check(request);
...
UniValue getspentinfo(const JSONRPCRequest& request)
{
            RPCHelpMan{"getspentinfo",
                ...
                { 
                  {"", RPCArg::Type::OBJ, RPCArg::Optional::NO, "",
                    {
                      {"txid",  RPCArg::Type::STR, RPCArg::Optional::NO, "The hex string of the txid"},
